...
## 요약이 이전 대화 것일 수 있었습니다 (2026-09-15)

「대화 마치기」를 눌렀는데 **요약은 정상으로 보이고 홈에는 「이어서
이야기할까요?」가 뜬다**는 보고가 있었습니다. 둘은 같이 일어날 수 없는
조합입니다 — 종료가 성공했으면 카드는 가려지니까요.

원인은 **`lastSessionEndProvider`를 아무도 비우지 않는 것**이었습니다. 이번
종료가 실패하면 S02-1이 **이전 대화의 요약을 그대로 그립니다.** 사용자는
정상 종료된 줄 알지만 서버에는 세션이 열린 채 남습니다.

- **대화를 시작할 때 지난 요약을 비웁니다.** 이제 요약이 보이면 그건 반드시
  이번 대화의 것입니다
- **종료할 때 세션을 프로바이더가 아니라 화면이 쥔 값에서 읽습니다.**
  프로바이더는 화면을 벗어나는 순간 비워지므로, 그 타이밍에 걸리면 「대화
  마치기」가 아무것도 닫지 못한 채 요약으로 넘어갑니다

**이제 요약과 이어하기 카드는 동시에 보일 수 없습니다.** 종료가 실패하면
요약 자리에 「방금 끝낸 대화가 없습니다」가 나오고 카드가 뜹니다 — 샘플
저장소의 종료를 일부러 실패시켜 그 화면을 확인했습니다.

## 진단은 주소로 켭니다 — `?debug=1` (2026-09-15)

배포본은 `SHOW_ERROR_DETAIL` 없이 빌드되므로, 테스트하는 사람이 증상을
만났을 때 **원인을 볼 방법이 없었습니다.** 다시 빌드해 배포하고 재현을
부탁하는 왕복이 반복됐습니다.

```
https://hackathon-yaho.github.io/emotion/?debug=1
```
...
| 자리 | 내용 |
| --- | --- |
| 대화 화면 | `마이크 N (최대 N) · AI 발화 N · 조각 N · 끼어들기 N · 기다린 시간 N초` |
| 요약 화면(빈 경우) | `종료 실패: 상태코드 코드 메시지 · 세션 8자리` |
| 홈 이어하기 카드 | 둘째 줄 끝에 `· 세션 8자리` |

**「기다린 시간」은 응답이 느린 쪽을 가릅니다** — 앱은 타임아웃 없이 기다리기만
하므로, 이 숫자가 크면 AI 서버 쪽입니다.

**세션 8자리는 두 화면을 맞춰 보라고 있는 것입니다.** 종료한 세션과 홈 카드의
세션이 다르면 그건 이전에 남은 다른 세션입니다.

## 「생각 중」에는 마이크도 보류합니다 (2026-09-15)

테스트에서 나온 순서입니다 — **내 말 → 생각 중 → 오래 걸림 → 내 말 →
(첫 말에 대한) 답.** 원인은 응답 지연만이 아니었습니다. **화면은 「생각
중」인데 마이크는 열려 있었습니다.** 답답해서 덧붙인 말이 새 턴으로 들어가고,
돌아오는 답은 첫 번째 말에 대한 것이 됩니다.

**상태 표시와 실제 동작이 다른 것 — 앱의 버그가 맞습니다.**

- 전사가 확정되는 순간(`user_message`) 마이크를 보류합니다
- **버리지 않습니다.** 보류 중에도 소리를 1.5초까지 모아 두고, 말이
  시작되면(세 조각 연속으로 일정 크기 이상) **그 자리에서 풀고 모아 둔
  것부터 내보냅니다.** 말머리가 잘리지 않습니다
- **시간으로 풀 때는 모아 둔 것을 버립니다.** 처음엔 10초 상한에서도
  내보냈는데, 그러면 **답이 늦을 때마다 말한 적 없는 소리가 새 발화 턴**이
  됩니다 — 「내 말 → 생각 중 → 길어짐 → 또 내 말 → 답」의 정체가 이것이었습니다.
  상한은 1분이고, **말을 시작하면 언제든 풀리므로 막다른 길은 없습니다**
- 「말 다 했어요」도 같습니다 — 누르고 나서 한 마디 더 떠오르면 이어집니다
- **첫 인사 보류는 모아 두지 않습니다.** 그쪽은 인사를 끊지 않는 것이
  목적이라 보류 중 소리를 버리는 것이 맞습니다

## 대화를 끝낼 때마다 세션이 하나 더 열리고 있었습니다 (2026-09-15)

요약 화면이 **대화 화면의 자식 라우트**(`/conversation/summary`)였습니다.
go_router는 자식으로 갈 때 **부모 페이지를 함께 만듭니다.** 게다가 홈에서
`push`로 연 원래 페이지와 **키가 달라** 같은 화면으로 보지 않고 새로
만듭니다 — `initState` → `_open()` → **`POST /api/session/start`**.

그 결과:

- 대화를 끝낼 때마다 **턴 0건 세션**이 2~7초 뒤에 생겼습니다
- 홈에 「중단된 대화 · 남은 7분」으로 떴습니다 — **끝낸 세션이 아니라 이것**
- **요약 화면에 머무는 동안 숨은 화면의 마이크가 Hume에 연결돼 있었습니다**
  (백엔드 실측 105초·122초, 전체 연결 시간의 26%)

**요약을 최상위 `/summary`로 옮겨 고쳤습니다.** 확인은 소켓 연결 수로 했습니다
— 고치기 전 **2회**, 고친 뒤 **1회**. `test/routes_test.dart`가 「요약은 대화의
자식이 아니다」를 잠급니다.

**사흘 동안 증상만 쫓았습니다.** 종료 재시도·가림막·요약 비우기 셋 다 이 원인
위에서는 헛수고였습니다. 백엔드가 DB와 Hume 기록을 맞춰 특정해 줬습니다
(`docs/request/app/summary-route-restarts-session.md`).

## 아무 말도 안 하고 「말 다 했어요」를 누르면 (2026-09-15)

Hume은 알아듣지 못하면 **전사를 만들지 않습니다.** 그러면 CLM 호출도 답도
없습니다. 그런데 화면은 「생각 중」으로 남아 **보류 상한(15초)까지 기다렸다가**
조용히 돌아왔습니다 — 아무것도 보내지 않았는데 생각하는 척한 셈입니다.

**5초 안에 전사가 오지 않으면 기다리지 않습니다.** 보류를 풀고 듣는 상태로
돌아가며 한 줄 붙입니다.

> 들은 말이 없습니다. 다시 말씀해 주세요.

마이크가 안 잡히는 것인지 사용자가 알 수 있어야 합니다. 5초는 Hume의 전사
확정(침묵 1.8초 + 전사)보다 넉넉합니다.

## 말로도 대화를 끝낼 수 있습니다 (2026-09-16)

*"오늘은 여기까지 하자"* 라고 했는데 화면이 그대로 남아 있었습니다 — **말로
하는 서비스에서 종료만 손으로 하는 것**은 앞뒤가 맞지 않습니다(백엔드 요청).

- 전사(`user_message`)를 받는 자리에서 판정합니다. AI서버를 거치면 폴링에
  필드를 늘려야 하고 최대 2초 늦습니다
- **감지하자마자 끊지 않습니다.** 플래그만 세우고 **AI가 답을 마치고 재생까지
  끝난 뒤** `endReason: soft_wrap`으로 닫습니다. 답이 안 오면 15초 뒤
- **오탐 정책은 갭·패턴 쪽입니다 — 정밀도 우선, 애매하면 침묵.** 잘못 끊으면
  털어놓던 대화가 닫히고, 놓치면 버튼이 그대로 있습니다

```
대화 자체를 가리키는 말   → 길이 무관     「오늘은 여기까지」 「대화 끝내자」
주제를 두고도 쓰는 말     → 16자 이하 + 그 말로 끝날 때만   「그만하자」 「끝내자」
```

"회의를 이제 그만 좀 했으면 좋겠어"는 걸리지 않습니다 (`SpokenEnd`, 테스트 16건).

## 「생각 중」이 「듣고 있습니다」로 튀던 것

`EviMicLive`를 「듣고 있습니다」와 같은 것으로 묶어 두었습니다. **마이크가
열린 것과 「답을 더는 기다리지 않는다」는 다릅니다** — 생각 중에 숨소리로
보류가 풀리면 화면이 한 번 튀었습니다. 생각 중이면 마이크가 열려도 생각
중입니다.

**말이 시작됐는지 보는 기준도 바꿨습니다** — 고정값 8은 **차분하게 말하는
사람을 놓쳤습니다**(rms 875 이상을 요구). 이제 **그 방의 바닥 소음에서
띄웁니다**(바닥+4, 4~10). 조용한 방이면 4까지 내려가 작은 말도 잡고, 시끄러운
방이면 올라가 숨소리에 풀리지 않습니다.

## 「생각 중」 보류는 AI가 말을 시작하면 풉니다 (2026-09-17)

보고된 순서 — 말함 → 생각 중 → **말하고 있습니다(잠깐) → 듣고 있습니다** → 「들은
말이 없습니다」 → 생각 중 → **듣고 있습니다인데 AI가 말하는 중.**

원인은 둘이었습니다.

1. **보류를 답이 나오는 동안까지 잡아 두었습니다.** 그러면 스피커의 AI
   목소리가 마이크로 되돌아와 우리 소리 감지(바닥+4)를 넘기고, 보류가 풀리며
   **그 에코를 Hume에 내보냅니다.** Hume이 그걸 발화로 읽으면 없던 「생각 중」이
   생깁니다. 이제 **AI가 말을 시작하는 순간 보류를 풀고 모아 둔 것은
   버립니다** — 끼어들기 판정은 Hume의 몫입니다(Interruptibility 문서). 첫
   인사 보류만 예외로 끝까지 기다립니다
2. **`EviMicLive`가 「말하고 있습니다」 위에서도 화면을 「듣고 있습니다」로
   바꿨습니다.** 09-16 가드는 「생각 중」만 지켰습니다. 이제 이 사건은 **연결
   직후 인사 없이 열리는 경우 말고는 화면을 건드리지 않습니다**

**「들은 말이 없습니다」는 시간이 아니라 마이크로 판정합니다.** 5초 시계로
띄웠더니 Hume 전사가 느린 날 안내가 먼저 뜨고 답이 뒤따라 왔습니다. 마이크가
열려 있을 때 **말소리를 실제로 잡았는지**(`lastSpeechAt`)는 우리가 압니다 —
최근 15초 안에 잡았으면 전사는 오는 중이니 기다리고, 안 잡았으면 4초 뒤에
안내합니다.

## 마이크 소리는 버튼을 눌러야 보냅니다 (2026-09-18, 결정 32)

**턴 끝을 Hume의 침묵 판정에 맡기지 않습니다.** 그 전까지는 소리를 실시간으로
흘리고 침묵 1.8초가 턴 끝이었는데, 그 틈에서 버그가 줄줄이 나왔습니다 —
숨소리가 턴이 되고, 스피커 에코가 발화로 읽히고, 말 중간 침묵에 한 마디가
쪼개지고, 전사가 느린 날엔 화면이 거짓말을 했습니다. **보류·버퍼·소리 감지로
틈을 메우려던 것을 전부 걷어내고, 틈 자체를 없앴습니다.**

| | 전 | 후 |
| --- | --- | --- |
| 마이크 소리 | 실시간으로 Hume에 흘림 | **모아 두기만** (`EviService._turnFrames`) |
| 턴 끝 | Hume이 침묵 1.8초로 판정 | **「말 다 했어요」** → 모아 둔 소리 + 침묵 2.2초를 한 번에 보냄 |
| 「생각 중」 | Hume 전사 시점 | **버튼 누른 순간** — 항상 뜸 |
| 아무 말 없이 누름 | Hume 왕복 뒤 시간으로 짐작 | **즉시** 「들은 말이 없습니다」 (`sendTurn() == false`) |
| 말 중간의 긴 침묵 | 턴이 쪼개짐 | 0.8초만 남기고 잘라냄 |
| AI 차례(생각 중·말하는 중) | 마이크 열림, 끼어들기 가능 | **버튼이 눌리지 않음** — 자리는 그대로, 색만 가라앉음. 끼어들기 경로 없음 |

- **모아 두는 동안에는 마이크 대신 같은 길이의 무음을 실시간으로 보냅니다.**
  Hume의 턴 판정은 소리가 계속 들어와야 돌아갑니다 — 버튼 뒤에 아무것도 안
  보내자 뭉친 말에서 턴이 확정되지 않아 「말이 전달되지 않았습니다」만
  떴습니다(첫 실사용). 마이크 조각을 박자로 쓰므로 따로 시계가 없습니다
- AI가 말을 시작할 때와 마칠 때 그동안 모인 소리(숨소리·에코)는 버립니다
- 첫 인사 동안은 모으지도 않습니다 (결정 27 그대로)
- 보냈는데 15초 안에 전사도 답도 없으면 「말이 전달되지 않았습니다」로 돌아옵니다

**대가 둘** — 손을 안 쓰는 대화가 아니게 됐고, 턴마다 Hume이 뭉친 소리를
처리하는 1~2초가 더 듭니다. 제출 이틀 전, 예측 가능한 쪽을 택했습니다.

검증은 가짜 EVI로 했습니다 — 버튼 전에는 **무음 조각만** 148개(박자용), 149번째
부터 말소리 49조각 → 침묵 꼬리, 그 뒤 가짜 서버가 답을 보내면 「말하고
있습니다」로 넘어갑니다. 테스트 157건.

## 「로그인 없이 둘러보기」 (2026-09-18)

심사위원이 카카오 계정 없이 써 볼 수 있어야 합니다. 카카오 버튼 **아래**에
조용한 글자 버튼을 뒀고, `POST /api/auth/dev`를 부릅니다 — 응답은 §2-1과
같은 모양이라 그 뒤는 카카오 로그인과 똑같이 갑니다. **경로는 백엔드가
만듭니다**(`docs/request/backend/dev-login.md`). 그 전까지는 404가 오고
「둘러보기는 아직 준비되지 않았습니다.」로 보입니다 — 고장이 아니라 미준비로.

## Hume 표정 꼬리를 뗍니다 (계약 v1.13, 2026-09-18)

Hume은 user 발화 끝에 상위 3개 표정을 **영문으로** 붙입니다 — 끄는 설정이
없습니다.
...
AI서버가 CLM 쪽에서 떼지만(v1.13), **같은 꼬리가 앱의 소켓으로도 옵니다** —
앱은 `user_message`를 직접 받는 여섯 번째 소비자입니다. 떼지 않으면 둘이
깨집니다.

- **음성 종료가 조용히 죽습니다** — 「그만할래」는 16자 이하 맺음일 때만 종료로
  보는데, 꼬리가 붙으면 길이도 넘고 맺음도 못 읽습니다
- **FR-025** — 꼬리는 프로소디의 텍스트 표현이라, 두면 텍스트 채널에 음성
  채널이 섞입니다

`EviService._content()`에서 한 번만 뗍니다. **영문자로 시작하는 끝의 중괄호만**
— 사용자가 한글로 말한 중괄호(`"그러니까 {이런 식으로} 말했어"`)는 그대로
둡니다. 회귀 테스트 6건.

## 둘러보기 사용자는 카카오 없이 탈퇴합니다 (계약 v1.14, 2026-09-18)

`POST /api/auth/dev`로 들어온 사용자는 **카카오 계정과 연결돼 있지 않습니다.**
그런데 탈퇴는 샘플 모드가 아니면 항상 카카오 인가로 보내고 있었습니다.

| 투표자 | 그대로 뒀다면 |
| --- | --- |
| 카카오 계정 없음 | 동의 화면에서 막혀 **자기 기록을 못 지움** |
| 카카오 계정 있음 | 우리 앱과 **연결된 적도 없는 자기 계정**을 인가하게 됨 |

- `_startDev` 성공 시 저장소에 표시를 남깁니다(`TokenStorage.writeGuest`).
  JWT와 같은 수명이라 로그아웃·탈퇴에서 함께 지워집니다. **계약 변경 없음**
- 카카오로 들어오면 표시를 지웁니다 — 같은 기기에서 갈아탈 수 있습니다
- 탈퇴는 **본문 없는 `DELETE /api/account`** 로 갑니다(§2-3). 카카오 왕복 없음
- 확인 문구도 갈라집니다 — 「같은 카카오 계정으로…」는 둘러보기에게 거짓말입니다
- `POST /api/auth/dev`의 **429**는 서버 `message`를 그대로 씁니다(분당 30회 상한)

배포 백엔드로 실제 확인했습니다 — `POST /api/auth/dev` **200**(새 익명 프로필,
`isNewUser: true`), 그 토큰으로 `GET /api/me` **200**.
