...
## 「새로 시작」이 이어하기였습니다 (2026-09-08)

홈의 이어하기 카드에서 **「새로 시작」과 「이어서 하기」가 완전히 같은 코드**였습니다.

```dart
onPressed: () => context.push(Routes.conversation),   // 두 버튼 다 이것
```

대화 화면은 열린 세션이 있으면 **무조건 `resume`을 부릅니다.** 그래서 「새로
시작」을 골라도 이전 세션이 이어졌고, 사용자에게는 **"잠깐 대화했는데 대화
시간이 10분"**으로 보였습니다 — 이어한 세션의 경과 시간입니다. AI가 인사하지
않은 것도 같은 이유입니다(이어하기에는 인사가 없습니다).

**계약 §2-5-1이 이미 정해 둔 것을 구현하지 않았습니다** — *"사용자가 아니오를
고르면 앱은 이 대신 `POST /api/session/{id}/end`(`endReason: user_end`)를
호출해 그 세션을 닫는다."*

- `startFreshProvider`로 의도를 전달합니다. **버튼이 두 개인데 경로가 하나면
  구별할 방법이 없습니다**
- `startFreshSession()`이 **닫고 나서 시작합니다.** 닫기가 실패해도 새로
  시작합니다 — 거기서 멈추면 사용자는 대화를 아예 못 하게 되고, 열린 세션은
  서버가 타임아웃으로 정리합니다
- 순서·실패·대기열 경로를 테스트로 잠갔습니다 (`test/session_entry_test.dart`)

헤드리스로 확인했습니다: 샘플에 열린 세션을 넣고 「새로 시작」을 누르면 링이
**「듣고 있습니다」**(새 대화)이고 「이어서 듣고 있습니다」가 아닙니다.

## 말이 오가지 않은 세션은 기록에 보이지 않습니다 (2026-09-09)

백엔드가 `durationSec` 정의를 고치며(계약 v1.11 — **첫 턴~마지막 턴, 하드컷
상한**) 턴 0건 세션은 **보일지 말지 앱이 정하라**고 넘겼습니다. 감춥니다.

- `SessionSummary.isEmpty`(`turnCount == 0`) 하나로 기록(S05)과 홈의 「지난
  대화」가 같은 기준을 씁니다
- **걸러낸 뒤에 빈 상태를 판정합니다** — 순서를 뒤집으면 빈 세션만 있을 때
  목록이 통째로 사라지고 빈 상태 문구도 안 나옵니다
- **지우지는 않습니다.** 사용자 데이터이고, 탈퇴 문구의 건수는 서버
  `sessionCount`를 그대로 씁니다 — "지워질 것"은 실제로 지워지는 전부여야
  합니다
- 페이징은 서버 `total`을 그대로 쓰므로 `hasMore`가 어긋나지 않습니다

`durationSec`은 이제 「말이 오간 시간」입니다. **앱은 고칠 것이 없습니다** —
숫자를 그대로 쓰면 되고, 문구도 그대로입니다.

## 2026-09-14 테스트 피드백 네 건

**① 인사가 나오기 전에 마이크가 열릴 때가 있었다 ("될 때도 있고 안 될 때도")**

`assistant_end`는 "조각을 다 보냈다"가 아니라 **메시지가 끝났다**는 뜻이라 그
뒤로도 `audio_output`이 옵니다. 큐가 **조각 사이에 잠깐 비는 순간**을 재생
끝으로 읽고 마이크를 열고 있었습니다 — 그래서 될 때도 있고 안 될 때도
있었습니다. 이제 **700ms 연속으로 조용하고 그동안 새 조각도 오지 않아야**
끝으로 인정합니다. 인사가 시작되기를 기다리는 유예도 2.5 → 3.5초.

가짜 EVI가 `assistant_end`를 먼저 보내고 조각을 2.1초에 걸쳐 흘리도록 만들어
확인했습니다 — 첫 마이크 프레임이 **마지막 조각 뒤 1326ms**에 나갑니다.

**② 내가 말한 내용이 0.5초 만에 떴다 사라졌다 → 자막을 아예 없앴습니다**

처음에는 "AI가 말하기 시작할 때 지운다"로 고쳤는데, 이어진 피드백이
**전사 품질이 고르지 않아 보기 싫다**였습니다. 그래서 **사용자 발화를 글로
띄우지 않습니다.** 링과 상태 문구만 남습니다.

`user_message`는 계속 씁니다 — **내용이 아니라 말이 끝났다는 사실**만 쓰고,
그것으로 「생각 중」에 들어갑니다. 그 자리에 남은 한 줄은 개발용 진단 문구
전용입니다(`SHOW_ERROR_DETAIL`).

**③④ 대화를 끝내고 홈에 왔는데 「이어서 이야기할까요?」가 떠 있었다**

`end` 응답을 받은 **뒤에** `/me`를 다시 부르는데도 서버가 그 세션을 아직 열려
있다고 답하는 순간이 있습니다(새로고침하면 사라졌습니다). **우리가 방금 닫은
세션이 무엇인지는 우리가 아니까** 그 id 하나만 홈에서 무시합니다
(`visibleOpenSession`). 다른 열린 세션은 그대로 보여줍니다.

**화면을 가리는 것이지 값이 맞아진 것은 아닙니다** — 원인은
`request/backend/open-session-used-time.md`에 같이 적어 백엔드에 넘겼습니다.

## 2026-09-15 — 「말하고 있습니다」가 깜빡이던 것 · 요약 화면에서 들리던 인사

**① 생각 중 → 말하는 중 → 듣는 중 → 말하는 중**

`assistant_end`를 받자마자 「듣고 있습니다」로 바꾸고 있었습니다. 그 프레임은
**소리가 끝났다는 뜻이 아닙니다** — 뒤로도 조각이 옵니다. 이제 **재생이 끝난
뒤에** `EviAssistantDone`을 냅니다(700ms 연속 조용 + 새 조각 없음). 마이크를
여는 신호와 같은 장치를 씁니다.

**② 요약 화면에서 첫 인사말이 들렸다**

`play()`는 비동기라 **`stop()` 뒤에 시작될 수 있습니다.** 대화를 끝내고 화면을
떠났는데 그 사이 시작된 조각이 그대로 흘러나온 것입니다. 재생 세대를 두고,
시작 시점에 세대가 달라져 있으면 **그 자리에서 다시 멈춥니다.**

둘 다 회귀 테스트로 잠갔습니다(`evi_test.dart` — 4건). 가짜 EVI가 답을 두
조각으로 나눠 보내게 해서 화면을 찍었습니다: 답 도착 → **assistant_end 뒤에도
「말하고 있습니다」 유지** → 마지막 조각 재생이 끝난 뒤에야 「듣고 있습니다」.
