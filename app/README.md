...
담당: **앱**. **Flutter · 웹 1순위** (2026-09-03 확정, 근거는 PRD §14-4).

- 대회 제출물의 서비스 링크가 **웹 URL**이라 브라우저가 1순위 타깃입니다. 레이아웃은 모바일 폭 기준으로 만들고, iOS·Android 빌드는 같은 코드베이스의 부산물로 둡니다.
...
| 항목 | 선택 | 비고 |
| --- | --- | --- |
| 프레임워크 | Flutter (stable 3.47.1에서 검증) | 타깃 우선순위: **웹 → Android → iOS** |
...
| 오디오 | 웹은 `record` + `audioplayers` (`kIsWeb` 분기) | PCM16 48kHz 모노, `echoCancel`·`noiseSuppress`·`autoGain` |
| 배포 | 정적 호스팅 (Vercel 등) | **HTTPS 필수** — 마이크 권한이 HTTPS/localhost에서만 열린다 |
...
빌드는 `flutter build web --release`. 초기 로드 실측은 `main.dart.js` 2.0MB + CanvasKit 5.2MB ≈ 7MB로, 제출 링크의 첫인상에 영향이 있어 축소를 검토 항목으로 둔다.
