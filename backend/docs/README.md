...
| **스케줄러 주기 (2026-09-04 ⑪)** | **5분, `fixedDelay`** | `phase-1-skeleton.md` 1-7 | F2-06이 "30분 경과"를 판정하므로 오차가 최대 5분이고, `resumableUntil = ended_at + 30분`이라 이어하기 창도 그만큼만 밀린다. **1분은 빈 스캔이 60배**(관찰은 3회·1.5배 조건이라 매번 안 생긴다), **10분은 이어하기 창이 40분까지 밀린다.** `fixedRate`가 아닌 이유는 배치가 길어질 때 겹쳐 도는 것을 막기 위함 |
