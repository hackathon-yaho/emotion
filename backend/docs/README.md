...
> **수정 기록 (2026-09-06 ㉛)** — **앱이 우리 결함을 하나 잡아 보냈다. 고쳐서 배포했다.** **필터가 만드는 401에만 CORS 헤더가 없었다**(`request/backend/cors-on-401.md`). 배포본에서 그대로 재현했다 — 200·400은 붙는데 401은 한 줄도 없다. **원인은 계층이다**: `CorsConfig`가 `WebMvcConfigurer`라 요청이 `DispatcherServlet`까지 가야 헤더가 붙는데, `JwtAuthFilter`는 그 앞에서 401을 만들고 끝낸다. **"400은 컨트롤러가·401은 필터가 만든다"는 앱의 짐작이 원인 그 자체**였다. 브라우저가 그 응답을 통째로 막으므로 앱에는 상태 코드도 `error.code`도 안 가고 **네트워크 실패로만** 보인다 — 계약 §1-2의 `TOKEN_EXPIRED`로 재로그인시키는 **F1-02가 구조적으로 안 불리고 있었다.** `CorsFilter`를 `HIGHEST_PRECEDENCE`로 인증 필터 앞에 뒀다(설정값은 그대로). 배포본에서 **401·없는 경로·`/internal` 401까지 전부 헤더가 붙는 것을 확인**했다. **테스트 2건 추가, 107건 통과.** **기존 프리플라이트 테스트 3건은 전부 통과하는 채로 이 결함이 배포돼 있었다** — `OPTIONS`는 인증 필터가 메서드로 제외해 MVC까지 가는데 본 요청의 401은 그 앞에서 끝난다. 같은 "CORS 테스트"인데 서로 다른 경로를 본다. `defects.md`에 **모양 ②·⑥**으로 올리고 "우리 것을 남이 잡아준 것" 절을 신설했다. 아울러 **AI서버가 Cloud Run에 배포돼 `AI_SERVER_BASE_URL`을 넣었다**(`https://emotion-ai-server-gq7yhdrrlq-du.a.run.app`). ⚠️ **킵얼라이브는 `/healthz`가 아니라 `/health`다** — Cloud Run이 `/healthz`만 앞단에서 가로채 구글 HTML 404를 낸다(우리도 실측했다). **AI서버 쪽 공유 시크릿이 아직 배포용이 아니라 양방향 401이다** — 그게 맞춰져야 대화가 성립한다.
>
> **수정 기록 (2026-09-06 ㉚)** — **앱 회신 2건. 재빌드는 됐는데 배포본이 백엔드를 안 부른다.** ① **진단이 맞았다** — `API_BASE_URL`(9/5 05:23)·`KAKAO_REST_KEY`(9/4 20:10) 등록이 마지막 빌드(9/4 19:38)보다 늦어 둘 다 안 실려 있었다. `workflow_dispatch`로 다시 돌렸고(run 34003227111), **우리도 배포본을 직접 확인했다** — `main.dart.js`에 `emotion-6yeh.onrender.com`이 있고 `localhost:8080`은 **0건**이다. ② **다만 앱이 `SAMPLE_DATA=true`를 일부러 켜 뒀다**(저장소 변수, 9/6 01:10 — 실재하는 것을 확인했다). **배포 링크가 팀 밖에서도 열리는데 라이브면 누르는 즉시 EVI 통화가 열리고 과금**되기 때문이다. 그래서 **`GET /api/health`를 포함해 백엔드 호출이 한 건도 안 나간다** — **서버 로그가 조용한 것이 정상이다.** 이걸 모르면 "배포했는데 왜 아무도 안 들어오지"를 엉뚱한 데서 찾게 된다. **`SAMPLE_DATA`는 제출 전에 반드시 꺼야 한다** — 켠 채로 제출하면 심사자가 준비된 데이터를 실제 기록으로 본다. ③ **CORS는 아직 실제 브라우저 왕복으로는 확인되지 않았다.** 우리가 `curl`로 프리플라이트를 확인한 것이 전부이고, 앱도 그렇게 적었다 — 라이브 전환일에 함께 본다. ④ **대기열 화면이 붙었다**(`session-queue.md` 회신) — **상태 코드로 가른다**(200 세션 / 202 티켓, 본문 모양으로 짐작하지 않는다) · `pollIntervalSec`를 상수로 박지 않았다 · 404는 조용히 §2-4부터 다시 · **`E0700`에 `busy`를 따로 만들어 `start`가 아니라 이어하기로 되돌아간다**(개정에서 짚은 그 분기다. 소켓 오류를 한 덩어리로 묶었으면 그대로 틀렸을 자리다). **서버 기본이 꺼짐이라 앱은 202를 실제로 받아본 적이 없다** — 켜는 날 202 → 폴링 → `position: 0` → 입장과 **새치기 방어**를 같이 보기로 했다.
>
> **수정 기록 (2026-09-05 ㉙)** — **AI 회신이 한꺼번에 왔고, 그 회신을 따라가다 결함 하나를 더 찾았다.** ① **우리 전제가 하나 틀렸다** — Gemini 모델명이 안 맞는다고 올린 근거가 **우리 로컬 `ai-server/.env`**였는데, 그건 `.gitignore` 대상이라 **사람마다 다른 파일**이고 공유되는 단일 출처는 `.env.example`이다. 그쪽 지적이 맞다. ② **그런데 회신이 안내한 "`.env.example`을 복사하세요"를 그대로 하면 안 된다** — **`.env.example`이 `config.py`보다 낡았다**(세 줄). 복사하면 **AI가 방금 고친 결함 둘이 되살아난다**: `AI_RESPOND_EFFORT=low`(코드는 `none`. **위기 응답의 109 안내가 문장 중간에서 잘린다** — FR-032·033이고 spec §11이 "F4는 어떤 스코프 컷에서도 자르지 않는다"고 못박은 자리다) · `AI_MODEL_OBSERVE=gemini-2.5-pro`(코드는 `gemini-3.8-flash`. **404, 신규 사용자 미제공**). **`.env`가 `config.py` 기본값을 덮으므로 코드에서 고친 것이 환경변수로 되돌아간다.** `defects.md`의 **모양 ③ "베낀 것이 어긋난다"** 그대로다 — 요청했다(`../../docs/request/ai/env-example-drift.md` ⏳). ③ **지연 수치가 정정됐다** — 우리가 올린 `latency-diagnosis.md`로 AI가 재측정했고 **합계 p95가 12.5초가 아니라 3.19초**로 나왔다(3.9배 과장). 원인은 **연속 호출 벤치마크가 429를 맞고 SDK 재시도로 잠든 시간**이 지연으로 기록된 것이었다. **팀이 틀린 근거로 유료 전환을 논할 뻔했다.** ④ **CLM은 등록이 막힌 게 아니라 넣을 주소가 없었다** — CLM URL은 공개 인터넷에서 닿아야 하는데 AI서버가 로컬에만 있었다. **AI서버를 Cloud Run에 배포하고 그 주소를 넣는다**(터널이면 끊길 때마다 Config를 고쳐야 한다). 비활성 **120 → 420초**. **`HUME_CONFIG_ID`는 그대로**라 백엔드 환경변수는 안 바뀐다. ⑤ **cron이 두 곳을 찔러야 한다** — **AI서버도 Cloud Run 무료라 잠든다.** 백엔드만 깨우면 이번엔 저쪽이 자고 있다. 슬립 대 fail-closed는 **cron이 맞다**고 확인됐다(복귀 60초를 타임아웃으로 덮으면 Hume이 먼저 끊는다). **쓰는 cron 서비스 이름을 AI에 알려줘야** 저쪽 것도 같은 도구에 등록한다.
>
> **수정 기록 (2026-09-05 ㉘)** — **배포했다 — `https://emotion-6yeh.onrender.com`.** `phase-7`의 "배포 당일 순서" 1~7과 9의 앱 절반을 실행했다. **계정 두 개는 이미 있었다**(Render 워크스페이스 `emotion` · Supabase `emotion`, ap-southeast-2). Render 서비스도 9/4에 만들어져 있었고 **그때 배포가 `update_failed`였던 원인은 `INTERNAL_SHARED_SECRET` 미설정**이었다 — 빌드는 성공했고 부팅에서 죽은 것이라 **로그 첫 화면에 그대로 찍혀 있었다.** 의도한 fail-fast가 실제로 그렇게 동작했다. ① **Supabase에 `migration.sql` 적용** — 로컬과 같은 파일, 11테이블 ② **배포용 시크릿 3개를 새로 생성**(`JWT_SECRET`·`TRANSCRIPT_ENC_KEY`·`INTERNAL_SHARED_SECRET`) — 로컬 값과 다르다 ③ **환경변수 11개 등록**(필수 10 + `KAKAO_CLIENT_SECRET`) ④ **부팅 98.7초**, HikariCP가 Supabase(Postgres 17.6)에 붙고 Tomcat이 `PORT=10000`으로 떴다. **실동작으로 확인한 것** — `GET /api/health` = `{"status":"ok","db":"ok"}` · **내부 API 인증이 시크릿 없음 401 · 틀림 401 · 맞음+없는 세션 404로 갈린다**(배포본에 `INTERNAL_SHARED_SECRET`이 실제로 물려 있다는 증거다 — 셋 중 하나만 봐서는 구분이 안 된다) · CORS 프리플라이트가 `https://hackathon-yaho.github.io`에 `access-control-allow-origin`을 돌려준다. **접속 문자열에서 함정 하나** — Supabase 비밀번호에 `@`가 들어 있어 연결 URL에 `@`가 두 번 나온다. **JDBC는 username·password를 별도 속성으로 받아 인코딩이 필요 없지만**, URL 하나로 넣는 방식이었으면 호스트 파싱이 깨졌을 자리다. 붙이기 전에 컨테이너 `psql`로 먼저 확인했다. **앱 쪽에서 하나 더 나왔다** — 저장소 변수를 등록해도 **지금 Pages에 떠 있는 빌드는 그 값을 모른다.** `app-web.yml`이 `--dart-define`으로 빌드 시점에 굽는데 **변수 등록(`API_BASE_URL` 9/5 05:23 · `KAKAO_REST_KEY` 9/4 20:10)이 마지막 성공 빌드(9/4 19:38)보다 늦다.** 즉 배포본이 폴백 `localhost:8080`을 부르고 카카오 키는 빈 문자열이라 **로그인부터 안 된다.** 워크플로는 `app/**` 변경에만 돌아 문서 푸시로는 안 걸리므로 **재빌드를 요청했다**(`../../docs/request/app/backend-deployed-rebuild.md` ⏳). **변수를 등록한 것과 그 값이 배포본에 들어간 것은 다른 일이다.** **문서 정정 1건** — `phase-7`이 두 곳에서 "필수 8개"라고 적었는데 표의 필수는 **10개**다(`HUME_SECRET_KEY`·`KAKAO_REST_API_KEY`가 늘 때 숫자가 안 따라갔다). **7-4에서 남은 체크박스는 cron 하나**이고, 그 밖에 사람 손이 필요한 것이 셋이다(`blocked.md` ⑦) — `TRANSCRIPT_ENC_KEY` 오프라인 사본 · AI에 새 `INTERNAL_SHARED_SECRET` 전달 · AI서버 배포 후 `AI_SERVER_BASE_URL`. **cron이 제일 급하다** — Render 무료는 15분 유휴에 잠들고 복귀 약 1분인데 **AI서버 세션 조회가 2초 fail-closed**라, 그 구간에 대화가 시작되면 CLM 인증이 막힌다. AI에 요청했다(`../../docs/request/ai/deploy-handoff.md` ⏳).
>
> **수정 기록 (2026-09-05 ㉗)** — **대기열을 자체 점검해 결함 4건을 찾아 고쳤다.** 만들자마자 [defects.md](defects.md)의 모양이 그대로 나왔다. ① **새치기** — `startOrEnqueue`가 `hasRoom()`만 봐서, **줄이 서 있는데 자리가 나면 새로 온 사람이 먼저 들어갔다.** 맨 앞이 계속 밀려 순번이 뜻을 잃는다. 줄이 비었을 때만 즉시 입장으로 고쳤다(**모양 ②** — 줄이 있는 상태는 정상 경로 테스트에서 안 밟힌다) ② **`DELETE`에 소유자 검사가 없었다** — 폴링은 검사하는데 취소는 안 해서 **남의 티켓을 줄에서 밀어낼 수 있었다.** 응답은 그대로 204다 ③ **계약 §2-4에 202가 없었다** — §2-14와 변경 이력에만 적혀 **앱이 §2-4만 보면 202를 모른다**(**모양 ③** — 한 곳만 고치고 참조 지점을 안 고쳤다) ④ **이어하기의 `E0700` 폴백이 틀렸다** — 앱 요청서에 "`start`부터 다시"라고 적었는데, **`start`는 열린 세션을 먼저 닫으므로 이어할 대화가 그 순간 사라진다.** 이어하기는 `resume` 재시도여야 한다. **이어하기가 대기열을 거치지 않는 이유**(이전 Hume 채팅이 비활성 2분에 이미 닫혀 자리가 비어 있다)도 계약에 적었다. **105건 통과**(+2). 앱 요청서는 회신 전이라 본문을 고치고 배너에 개정을 남겼다.
>
> **수정 기록 (2026-09-05 ㉖)** — **[defects.md](defects.md) 신설.** 결함 기록이 이 배너 26개에 흩어져 있어 **"같은 모양이 몇 번 반복됐는가"가 안 보인다.** 그게 배너가 못 주는 정보이고 이 파일이 담는 유일한 것이다. **반복된 모양은 여섯 개뿐이었다** — ① **조용히 실패한다**(가장 많았다. 실패한 쪽이 null이고 그 null이 계약상 허용값이다) ② **정상 경로에서는 안 밟히는 코드**(오류 경로·기본값 경로. `range` 생략처럼 **기본값이 실서비스의 정상 경로**인 경우가 많다) ③ **베낀 것이 어긋난다**(고치는 방향은 규칙을 더하는 게 아니라 **베낀 것을 지우는 것**) ④ **대역으로는 안 보인다**(목으로 확인한 것은 "돈다"가 아니라 "우리 쪽 코드가 돈다"이다) ⑤ **값의 수명이 서로 다르다**(정원은 Hume에, 마지막 활동은 턴에 묻는다) ⑥ **테스트가 자기 자신을 속인다**(롤백·격리·기본값처럼 **"안 일어나는 것"을 확인하는 테스트**가 위험하다). 코드 전 문서 대조에서 11건, 구현하면서 15건을 잡았고, **단계마다 나오는 결함의 종류가 달랐다**는 것이 시간순 표의 유일한 쓸모다. AI서버 결함 2건은 가리키기만 했다.
>
> **수정 기록 (2026-09-05 ㉕)** — **동시 접속 대기열을 만들었다 (계약 v1.9 §2-14). 기본은 꺼짐이다.** Hume 요금표의 동시 접속은 Free **1** / Starter·Creator **5** / Pro 10이고, **상한을 넘긴 연결은 대기가 아니라 `E0700` 거절**이다(Hume 오류 문서) — 순번·대기 시간이 Hume에서 오지 않으므로 대기열을 우리가 만들었다. ① **정원 판정은 Hume에 직접 묻는다**(`GET /v0/evi/chats?status=ACTIVE`, 실측 200). **우리 열린 세션 행을 세면 안 된다** — Hume은 비활성 **2분**에 채팅을 닫는데 `voice_session`은 **30분**을 열어 두므로 **실제로 비어 있는 자리를 두고 사람을 돌려보낸다** ② **예약 단계가 없다** — 폴링 응답의 `position: 0`에 세션이 동봉되고 **그 응답이 곧 입장권**이다. 자리를 미리 잡지 않으니 만료 타이머도, 가로챌 자리도 없다 ③ **줄 맨 앞의 폴링에서만 Hume에 묻는다** — 대기자가 늘어도 외부 호출은 폴링 간격당 한 번이다 ④ **조회 실패는 입장 허용(fail-open)** — 조회가 죽었다고 대화를 막지 않는다. 넘겨 붙으면 Hume이 `E0700`으로 거절하고 앱이 다시 태운다 ⑤ **인메모리 + 폴링 10초 끊기면 만료** — 브라우저를 닫은 사람이 줄을 막지 않게. 테이블·마이그레이션·정리 스케줄러를 늘리지 않았다(줄이 있다는 것은 서버가 한가하지 않다는 뜻이라 Render가 재우지 않는다) ⑥ **꺼진 경로를 따로 테스트했다** — 기본값이 꺼짐이라 **평소에 도는 쪽이 그쪽**인데, 켠 경로만 검증하면 정상 경로가 검증되지 않는다. `verify(never()).activeCount()`로 외부 호출이 안 나가는 것까지 못 박았다. **103건 통과**(+10). 앱 요청 `request/app/session-queue.md`.
>
> **수정 기록 (2026-09-05 ㉔)** — **Hume 키가 들어와 처음으로 실물에 붙었다. EVI 시간은 0초 썼다**(소켓을 안 열었다 — 토큰 발급·Config 조회·`chats?status=ACTIVE`는 전부 REST라 분수를 안 깎는다). ① **`POST /api/session/start`가 실물 토큰으로 201**이다. `blocked.md` ①의 첫 항목이 닫혔다 ② **결함 1건** — **Hume 토큰 엔드포인트가 200에 `Content-Type`을 안 싣는다.** 응답을 `Map`으로 받으면 Spring이 변환기를 못 골라 `UnknownContentTypeException`이 나는데, **겉으로는 키가 틀린 것과 똑같은 503**이다. 배포 후에 만났으면 키부터 의심했을 것이고, **대역으로는 절대 안 보이는 종류**다. `String`으로 받아 직접 파싱하도록 고치고, `Content-Type` 없는 200을 재현하는 회귀 테스트로 묶었다(`RestClient.Builder` 주입으로 바꿔 `MockRestServiceServer`가 붙는다). **93건 통과**(+3) ③ **Config가 아직 제품을 못 돌린다** — `language_model`이 **`null`**이라 **CLM이 안 붙어 있다.** 이대로면 Hume이 내장 LLM으로 대답하고 **우리 AI서버는 한 번도 안 불린다.** 그 밖에 영어 음성(Kora)·**비활성 타임아웃 120초**(감정 대화에서 2분 침묵은 흔한데 하드컷 7분보다 먼저 끊긴다)·**3초 nudges**. 실제 Config id는 `23d6162d-d334-44dd-9c2d-f5858de19a06`이고 배포 환경변수에도 이 값을 넣는다 ④ **동시 접속 실측** — 요금표 기준 Free **1** / Starter·Creator **5** / Pro 10. **Creator로 올려도 동시 접속은 안 늘고**(5로 같다) 포함 분수만 40→200분이다.
>
> **수정 기록 (2026-09-05 ㉓)** — **앱 회신 2건을 반영해 계약을 v1.8로 올렸다.** ① **`POST /api/session/{id}/chat-group` 신설**(§2-5-2) — 앱이 EVI 소켓 직후 `chat_metadata`로 받는 `chat_group_id`를 올린다. **`end` 본문이 아니라 별도 엔드포인트인 이유**는 앱이 강제 종료되면 `end` 호출 자체가 없는데 **이어하기가 필요한 상황이 정확히 그 상황**이기 때문이다. **멱등·204**, 종료된 세션에도 받고(도착 전에 닫힐 수 있는데 404로 튕기면 재시도가 영영 성공하지 못한다), 값 형식은 검사하지 않는다(Hume이 접두사를 바꾸는 날 우리가 먼저 깨진다). 이걸로 **`resumedChatGroupId`가 항상 null**이던 구멍이 닫혔다 ② **빈 값을 `null`로 확정**(§1-3에 행 추가) — 앱이 빈 문자열을 "없음"으로 방어하고 있었다. **응답 본문에 `"resumedChatGroupId":null`이 실제로 실리는지를 테스트로 고정**했다. 전역 `NON_NULL`을 켜면 그 테스트가 깨지고, 그 순간 앱의 방어가 조용히 무의미해지는 것을 막는다. **90건 통과**(+6). ③ **`KAKAO_REST_KEY` 저장소 변수를 등록했다** — 확인해 보니 **저장소 변수가 하나도 없었다.** 앱 웹 빌드가 `--dart-define=KAKAO_REST_KEY=` 빈 값으로 나가고 있었다. **`API_BASE_URL`도 비어서 폴백 `http://localhost:8080`으로 빌드된다** — 배포본이 조용히 로컬을 부른다. 배포 순서 9번에 등록 명령을 적어 뒀다.
>
> **수정 기록 (2026-09-05 ㉒)** — **Phase 7 준비 완료. 남은 것은 계정 두 개(Supabase·Render)뿐이다.** **배포 이미지로 실제 확인했다** — Docker 빌드 → `PORT=10000` 주입 기동 → DB 연결 → 헬스체크 → **TC-14 데모 모드 전환**까지. **배포 당일에야 드러났을 결함 둘을 미리 잡았다** — ① **`gradlew`에 실행 비트가 없었다**(git `100644`). 리눅스에서 `./gradlew`가 permission denied로 죽어 **빌드 자체가 안 된다.** `git update-index --chmod=+x`로 올리고 `Dockerfile`에도 `chmod +x`를 넣었다(Windows에서 다시 추가되면 또 사라지므로 두 겹) ② **문서의 환경변수 목록이 낡아 있었다** — 계약 v1.6에서 없어진 `KAKAO_APP_ID`가 남아 있고 **필수인 `KAKAO_REST_API_KEY`가 빠져 있었다.** 그대로 등록했으면 **부팅에서 죽는다.** 목록을 `application.yml`에서 뽑아 필수/선택으로 다시 만들었다(필수 8개). **7-2 로그 감사 통과** — 코드 전수에서 `transcript`·`sessionId` 0건, 음성 파일 코드 0건, 실행 로그 전수에서 백엔드 UUID 0건. **`open-in-view: false`로 바꿨다** — 응답 직렬화 동안 커넥션을 쥐지 않는다. Supabase Session pooler는 커넥션이 넉넉하지 않아 쥐는 시간이 곧 상한이 된다. 서비스가 트랜잭션 안에서 DTO를 다 만들어 지연 로딩에 기대는 곳이 없다(84건 통과). **배포 당일 순서 10단계**를 `phase-7`에 적었다 — **10번(`AI_SERVER_BASE_URL` 갱신)을 빠뜨리면 요약이 `null`, 관찰이 0건이 되는데 둘 다 정상 동작과 구분이 안 된다.**
>
> **수정 기록 (2026-09-05 ㉑)** — **[blocked.md](blocked.md) 신설.** 막혀 있는 것을 **"무엇이 막는가"로 묶었다** — Phase 문서의 `⏳`는 흩어져 있어서 "Hume 계정 하나가 4건을 막고 있다"가 안 보인다. 그게 grep이 못 주는 정보이고 이 파일이 담는 유일한 것이다. **개수는 적지 않았다.** 아울러 `phase-2`의 요약 생성 체크박스가 낡아 있던 것을 닫았다 — Phase 3에서 구현하고 2026-09-05에 성공 경로까지 확인했는데 Phase 2 문서에 `- [ ]`로 남아 있었다.
>
> **수정 기록 (2026-09-05 ⑳)** — **막혀 있던 것 중 2건은 상대가 아니라 도구가 없어서였다.** 스크래치패드에 20~40줄짜리 도구 둘을 만들어 닫았다. ① **재시도 시나리오(Phase 3 완료 기준)** — **고장 주입 프록시**가 `POST /internal/turns`를 백엔드로 넘긴 뒤 **503으로 바꿔** 돌려준다. AI서버는 턴마다 4회(최초 1 + 재시도 3)를 보냈고 프록시가 버린 백엔드 응답은 전부 202였다. **같은 턴을 총 8회 받고도 행은 2개, `TURN_INDEX_COLLISION` 0건** — AI가 제안한 시나리오가 실제로 돌았다. **백엔드에 고장 스위치를 넣지 않았다**(그런 코드는 배포까지 따라간다) ② **관찰·요약의 성공 경로(Phase 2·4)** — 진짜 AI서버는 키가 없어 422라 **실패 경로만 확인되고 성공 경로는 한 번도 안 돌았다.** 단위 테스트는 클라이언트 자체를 목으로 바꿔 **HTTP 계층이 실행된 적이 없었다.** 대역 AI서버를 세워 `회의`(갭 1.50 × 3)·`산책`(갭 0.10 × 3)을 넣으니 **`회의`만 관찰이 되고 `산책`은 침묵**했다 — `evidence 3 · 1.50 · 0.80 · 1.88`, 근거 3건 연결, 목록에 그대로. 요약도 종료 응답에 실렸다. **그 과정에서 결함 하나를 잡았다** — **`gapAvg`가 `0.80000000000000000000`으로 나가고 있었다.** Postgres `avg()`의 큰 스케일을 그대로 내보낸 것으로 계약 §1-1(소수 2자리) 위반이다. **DB에서 값이 나오는 자리에서 반올림**하도록 고쳤다(호출자마다 기억하게 두면 한 곳이 빠지는 순간 화면에 뜬다). 회귀 테스트 추가, **84건 통과**.
>
...
> **[blocked.md](blocked.md)는 "무엇이 막는가"로 묶은 스냅샷이지 단일 출처가 아니다.** Phase 문서가 출처이고, 그 파일은 **grep이 못 주는 것**(하나가 풀리면 몇 개가 같이 열리는가)만 담는다. 개수는 어느 쪽에도 적지 않는다.
...
| [blocked.md](blocked.md) | **막혀 있는 것을 "무엇이 막는가"로 묶은 스냅샷** — 값 하나가 오면 몇 개가 같이 열리는지 | 다음에 뭘 할지 정할 때 · 상대에게 무엇을 재촉할지 정할 때 |
| [defects.md](defects.md) | **끝난 결함의 기록 — 반복된 모양 여섯 가지** | **새 코드를 짜기 전** · 무엇을 먼저 검증할지 정할 때 |
