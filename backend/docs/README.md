...
> **수정 기록 (2026-09-05 ⑲)** — **통합 1·2번 실행.** 터널 대신 **양쪽을 로컬에서** 띄웠다(`ai-server/`가 같은 저장소에 있다). CLM 요청 → AI의 세션 조회 200 → CLM 200 SSE → **턴 2건 적재**까지 실제로 갔고, 프로소디 규칙이 `voiceValence −0.81`을 계산했다. `ANTHROPIC_API_KEY`가 없어 분석·응답·요약·관찰은 전부 실패했는데 **degraded 경로가 계약대로** 돌았다(`textValence: null`·`gap: null`, `summary: null`, 관찰 0건). **백엔드를 내리고 CLM을 쏴도 200** — fire-and-forget 성립. **AI 쪽 결함 2건을 보고했다**(`request/ai/integration-round-1.md`) — ① fail-closed 401 경로가 `error_log()` 인자 충돌로 **500**이 된다(정상 경로에서는 안 밟히는 코드다) ② **로그에 `sessionId` 평문 7건**(계약 §1-1 위반, 백엔드는 같은 시간대 0건). **우리 쪽 정정 1건** — `AI_SERVER_BASE_URL` 기본값을 8000 → **8100**으로. 계약에 포트 규정이 없어 상대 `.env.example`의 `AI_PORT`가 출처다. **못 본 것** — "저장 성공 + 응답 유실" 재시도 시나리오는 **고장 주입점이 있어야** 만들 수 있다. 백엔드를 내려 재시도가 돌고 3회 후 포기하는 것까지는 봤다.
>
> **수정 기록 (2026-09-05 ⑱)** — **Phase 6 구현 완료.** `DELETE /api/sessions/{id}`(F10-01·02) · `DELETE /api/account`(F10-03). **테스트 83건 통과**, 롤백까지 확인했다. **문서 순서를 두 곳 고쳤다** — ① **⑧(baseline 재계산)을 ⑤(관찰 재계산)보다 먼저** 돌린다. ⑤가 `ratio`를 다시 계산하는데 분모가 그 값이라, 나중에 갱신하면 **비율이 옛 평균으로 매겨진 채 남고** 관찰 evidence와 트렌드의 `userAvgGap`이 갈린다 ② **⑤에서 `user_avg_gap`도 갱신한다.** 문서는 세 필드만 적었지만 분모가 바뀌었는데 분모를 안 적으면 evidence 안에서 숫자가 안 맞는다(관찰 문장에는 숫자가 없어 문장과는 어긋나지 않는다). **탈퇴에서 잡은 함정 하나** — `account_profile`을 먼저 지우면 **어느 `account`였는지 알 방법이 없다.** F10-01의 ①과 같은 함정이라 `account_id`를 먼저 잡는다. **unlink는 트랜잭션 밖에서** 한다 — 한 트랜잭션에 묶으면 unlink 성공 후 커밋 실패로 "카카오는 끊겼는데 데이터는 남은" 상태가 생긴다. **롤백 검증만 트랜잭션 밖에서 돌린다** — 테스트와 서비스가 한 트랜잭션을 공유하면 서비스의 롤백이 테스트에 안 보여서, **정작 확인하려는 경계가 확인되지 않는다.**
>
...
| --- | --- | --- | --- |
| 삭제 순서 ⑧↔⑤ (2026-09-05) | **baseline 재계산을 관찰 재계산보다 먼저** | `phase-6` 완료 기준, `data-model.md` | ⑤가 계산하는 `ratio`의 분모가 baseline이다. 나중에 갱신하면 비율이 옛 평균으로 매겨진 채 남아 evidence와 트렌드가 갈린다 |
| 탈퇴 unlink 위치 (2026-09-05) | **트랜잭션 밖, 삭제 커밋 뒤** | `phase-6` 6-3, `AuthService` | 한 트랜잭션이면 unlink 성공 후 커밋 실패로 "카카오는 끊겼는데 데이터는 남은" 상태가 생긴다 |
