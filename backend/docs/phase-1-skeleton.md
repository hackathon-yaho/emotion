...
- ✅ 카카오 로그인 → JWT 발급 → 그 JWT로 `GET /api/me` 호출이 통과한다 — **인가 코드 방식으로 다시 검증했다**(2026-09-05, 계약 v1.6). 서버가 코드를 교환하는 실제 경로다
- ✅ **같은 인가 코드를 두 번 쓰면 401이다** — 1회용이라 카카오가 거절한다. 앱이 주소창의 `?code=`를 지워야 하는 이유가 이것이다(새로고침이 같은 코드를 다시 보낸다)
...
- ✅ **TC-01** — 재로그인 시 **동일 `profileId`** · `isNewUser: false` · 계정 행이 늘지 않음 (2026-09-04 실계정 검증. 같은 토큰 재사용 1회 + 리프레시로 받은 새 토큰 1회, 둘 다 동일)
  - **로그인 방식이 바뀌어도 같은 `profileId`가 나왔다** (2026-09-05). 액세스 토큰 방식으로 만든 계정에 인가 코드 방식으로 다시 들어가 `98c2fece…`가 그대로 나왔다 — 저장하는 값이 카카오 회원번호 하나라서다. **도그푸딩 중에 방식을 바꿔도 기존 데이터가 끊기지 않는다**
