...
> 남은 것은 **실제 카카오 값 두 개**뿐이다 — `KAKAO_APP_ID`(콘솔의 앱 ID, 지금 `.env`는 자리표시 `999999`)와 앱의 `KAKAO_JS_KEY`. 그래서 **카카오 로그인의 끝단(실제 토큰으로 로그인 → TC-01)만 미검증**이고, 나머지(기동·DB·11테이블·JWT·CORS·헬스체크·스케줄러)는 검증했다.
