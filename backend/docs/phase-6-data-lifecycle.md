...
- [ ] 응답 **204**
- [ ] **카카오 연결 해제(unlink)는 백엔드가 하지 않는다** — 아래

> **카카오 연결 해제는 앱이 한다 (2026-09-04 확정).** 백엔드가 204를 준 **뒤에** 앱이 카카오 SDK로 `unlink`를 호출한다. 순서가 이 방향인 이유는, **뒤집으면 unlink는 됐는데 DB 삭제가 실패하는 상태**가 생기기 때문이다 — 사용자 데이터가 먼저 사라져야 한다. unlink가 실패해도 우리 데이터는 이미 없으므로 탈퇴 자체는 성립한다.
>
