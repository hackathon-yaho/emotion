...
```sh
git pull && grep -L "상태: ✅" docs/request/{내 역할}/*.md   # README.md는 무시
```

> **`"✅"`가 아니라 `"상태: ✅"`로 찾는다** (2026-09-05 정정). 배너가 아니라 본문에 ✅가 한 번만 나와도 그 문서가 목록에서 빠진다 — 실제로 백엔드가 다른 요청을 가리키며 "(✅ 회신 완료)"라고 적었다가 **자기가 방금 올린 미회신 요청이 안 보이는** 일이 있었다. 배너 형식은 `> **상태: ⏳ 회신 대기**`로 고정이다(`docs/request/app/README.md`).
...
- **`ai-server/app/rules/` 순수 함수 계층 구현 완료(2026-09-04). 테스트 111건 통과** — valence·gap·crisis·tags·guard·observe_guard·summary_guard·turns. 나머지 계층(`clm/`·`llm/`·`session.py`·`main.py`)은 미착수
