...
> **상태: ✅ 회신 완료** (요청 2026-09-05 · 회신 2026-09-06)
> 회신은 [`../../response/backend/backend-deployed-rebuild.md`](../../response/backend/backend-deployed-rebuild.md)에 있습니다.
> **결론**: **다시 빌드했습니다**(run 34003227111) — 배포본에 `emotion-6yeh.onrender.com`이 실렸고 `localhost:8080`은 없습니다. 진단하신 "변수 등록이 마지막 빌드보다 늦다"가 정확했습니다. **다만 `SAMPLE_DATA=true`를 일부러 켜 두었습니다** — 배포 링크는 팀 밖에서도 열리고, 라이브면 누르는 즉시 실제 EVI 통화가 열려 과금됩니다. **그래서 지금은 백엔드 호출이 한 건도 나가지 않습니다**(서버 로그가 조용한 것이 정상). 끄는 건 변수 하나이고, 저희가 로컬에서 실제 왕복을 확인한 뒤 켜겠습니다.
