...
> **상태: ✅ 해소됨 (2026-09-07) — 올리는 사이에 앱이 먼저 고쳤습니다**
> 커밋 `03110b9` — `SAMPLE_DATA`를 끄고 서비스 워커도 비활성화하셨습니다. **회신은 필요 없습니다.**
> 아래 내용은 그때의 판단 근거로 남겨 둡니다. 다만 한 가지는 여전히 유효합니다 — **이제 배포 링크를 여는 누구나 실제 세션을 열 수 있고, 그건 곧 Hume 분수입니다.** 무료 티어가 월 5분이라 링크가 팀 밖으로 새면 그날 소진됩니다. `?sample=0` 방식(기본은 샘플, 아는 사람만 라이브)이 그 점에서는 더 안전하니, 5분을 다 쓰고 나면 되돌리는 것을 고려해 주세요.
>
> <sub>원래 배너</sub>
