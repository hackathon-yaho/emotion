...
>
> **수정 기록 (2026-09-08 ⑨, 백엔드)** — **v1.11. `durationSec`이 무엇을 재는 값인지 적혀 있지 않았고, 실제로는 종료 경로마다 다른 값이 들어가고 있었다.** 앱이 「잠깐 한 대화가 10분으로 나온다」고 요청해 확인했다(`request/backend/duration-definition.md`). ① **정의를 §2-5에 명시** — **첫 턴부터 마지막 턴까지**, `hardCutSec` 상한, 턴 0·1건이면 0. §2-9·§2-10의 같은 필드도 같은 값이다 ② **`usedSec`(§2-2)과 다르다는 것을 못 박았다** — `usedSec`은 **시작부터** 재는 잔여 시간 계산용, `durationSec`은 **첫 턴부터** 재는 표시용이다 ③ **종료 경로 간 불일치를 없앴다** — 사용자 종료는 벽시계(`ended_at − started_at`), 스케줄러 정리는 `usedSec`으로 서로 달랐고, 같은 컬럼에 두 정의가 섞여 들어갔다. **벽시계 쪽은 `hardCutSec` 420을 넘는 값까지 기록했다**(실측 951·630초). ⚠️ **하드컷으로 자르는 방안은 택하지 않았다** — 그러면 **서로 다른 대화가 전부 정확히 420초로 찍혀** 틀린 줄조차 모르게 된다. 실측 세션에서 **951초 → 31초**로 바로잡힌다. **필드 추가·삭제·개명 없음**(정의 명시 + 계산 변경). 테스트 109건.
>
> **수정 기록 (2026-09-07 ⑧, AI)** — **v1.10. 계약서가 틀렸고 앱은 그대로 구현했다.** §1-1이 `custom_session_id`를 **"EVI 소켓 URL"**로 보내는 것처럼 적어 두었는데, **Hume은 소켓 URL의 그 쿼리를 받지 않는다.** 그래서 Hume이 우리 CLM을 부를 때 값이 실리지 않고, §4대로 401을 내면 EVI가 그 자리에서 대화를 끊는다 — **실측 401 열 건, 전부 쿼리 없는 주소, 2ms, 대화 길이 1.5초.** ① **§1-1의 그 문장을 정정**했다(값 노출 사정 자체는 그대로다) ② **§4 표에 「세션 식별자 설정」 행과 경고를 신설** — 설정 경로는 **앱의 `session_settings` 메시지**와 **CLM 응답의 `system_fingerprint`** 둘뿐이고, 표에 적힌 `?custom_session_id=`는 **값이 설정된 뒤 Hume이 붙여 주는 것**이지 우리가 붙이는 것이 아니다. **이 실패가 연결 실패로 안 보인다는 점**도 적었다 — 소켓은 붙고 `chat_metadata`도 오고 세션 행도 생긴다. **§4는 "Hume이 정한 규격"이라 우리 판단이 아니라 사실 확인의 문제였고, 그 확인이 AI 쪽에서 빠졌다.** 앱에 요청 1건(`request/app/custom-session-id-not-reaching-clm.md`). **필드 추가·삭제·개명 없음.**
>
...
> ⚠️ **`custom_session_id`는 EVI 소켓 URL이 아니라 `session_settings`로 보낸다** (v1.10, 2026-09-07 — **정정**). 이 자리에 "EVI 소켓 URL의 `custom_session_id` 노출은 불가피하다"고 적혀 있었고 **앱은 그대로 구현했는데, Hume은 소켓 URL의 그 쿼리를 받지 않는다.** 설정 경로는 §4를 따른다. **값이 앱 쪽에 노출되는 사정 자체는 그대로다** — `language_model_api_key`를 앱에 내리지 않는 대신 택한 인증 수단이라 어느 경로로 보내든 번들에서 관측할 수 있고, 그래서 §3-4가 fail-closed로 막는다.
...
| 필드 | 설명 |
| --- | --- |
| `openSession` | **비정상 중단으로 열려 있는 세션.** 없으면 `null`. **턴이 0건이면 열려 있어도 `null`이다**(v1.12) — 이어갈 대화가 없다 |
| `usedSec` | **시작부터 마지막 턴까지**(v1.12 명시). 벽시계(`now − startedAt`)가 아니다 — 앱이 죽은 뒤 흐른 시간을 넣으면 5분 뒤 이어하기가 0초가 된다(spec TC-22). 마지막 턴 뒤 꼬리는 빠진다(실측 Hume 연결 대비 7~10초 적다) |
...
**앱은 상태 코드로 가른다** — `201`이면 위 응답 그대로, **`202`면 §2-14 대기열**이다. 이때는 세션도 만들어지지 않고 Hume 토큰도 발급되지 않는다.

> **`SESSION_QUEUE_ENABLED`가 꺼져 있으면 202가 나가지 않는다**(기본 꺼짐). 켜기 전에는 이 엔드포인트의 동작이 v1.8과 같다.
>
> **줄이 서 있으면 자리가 나도 202다.** 기다리던 사람보다 먼저 들어가지 못한다.
...
> **`durationSec`은 「말이 오간 시간」이다 — 화면을 열어 둔 시간이 아니다** (v1.11 확정). **첫 턴부터 마지막 턴까지**이고 `hardCutSec`을 넘지 않는다. **턴이 0·1건이면 0**이다. §2-9 목록·§2-10 상세의 같은 필드도 같은 값이다.
>
> **`usedSec`(§2-2)과 다르다.** `usedSec`은 **시작부터** 재서 잔여 시간을 계산하는 값이고, `durationSec`은 **첫 턴부터** 재서 화면에 보여줄 값이다. 둘을 섞어 쓰면 안 된다.
>
> ⚠️ **v1.11 전에는 벽시계(`ended_at − started_at`)였다.** 사용자가 화면을 열어 두고 한참 뒤에 말을 시작하면 그 공백이 전부 들어가, **31초짜리 대화가 951초로 기록됐다**(2026-09-08 실측, 앱 요청 `request/backend/duration-definition.md`). `hardCutSec`으로 자르는 방안도 검토했으나 **서로 다른 대화가 전부 정확히 420초로 찍혀** 틀린 줄조차 모르게 되므로 택하지 않았다. **종료 경로마다 정의가 달랐던 것**(사용자 종료는 벽시계, 스케줄러 정리는 `usedSec`)도 이때 함께 없앴다.
>
> **첫 발화 이전과 마지막 발화의 꼬리는 빠지므로 약간 과소하다.** 정확히 하려면 발화 길이가 필요하고, 그것은 §3-2와 스키마를 같이 바꿔야 한다 — 지금은 하지 않는다.
...
`tags`는 그 세션에서 가장 많이 등장한 상위 3개까지. **`durationSec`의 정의는 §2-5**(첫 턴부터 마지막 턴까지).
...
>
> **줄이 서 있으면 신규 시작도 202다.** 자리가 나도 §2-4가 새로 온 사람을 먼저 들이지 않는다 — 그러면 맨 앞이 계속 밀려 순번이 뜻을 잃는다.
>
> **`DELETE`도 소유자만 할 수 있다.** 남의 티켓을 줄에서 밀어낼 수 없다(그래도 응답은 204다).
>
> **§2-5-1 이어하기는 이 대기열을 거치지 않는다.** 이어하기는 중단된 대화로 돌아가는 것이고, 그 사람의 이전 Hume 채팅은 비활성 2분에 이미 닫혀 자리가 비어 있는 것이 보통이다. **정원이 찬 순간에 이어하기하면 소켓에서 `E0700`이 온다** — 그때 앱은 §2-4로 새 세션을 시작하지 말고 **잠시 뒤 이어하기를 다시** 시도한다. 새로 시작하면 그 순간 중단된 세션이 닫혀 **이어할 대화가 사라진다.**
>
...
| `transcript` | **사용자가 실제로 말한 문장.** Hume이 user 발화 끝에 덧붙이는 영문 표정 요약(`{slightly doubtful, slightly calm, …}`)은 **AI서버가 CLM 파싱 시점에 떼어낸 뒤** 보낸다(§4). 그 꼬리는 프로소디의 텍스트 표현이라, 남기면 텍스트 채널에 음성 채널이 섞이고(FR-025) 사용자가 말한 적 없는 문장이 기록된다. **백엔드는 받은 값을 고치지 않는다** — 원문의 정의를 한 곳에 둔다 (v1.13) |
...
| **`content`의 꼬리** | **user 발화 끝에 상위 3개 표정이 영문으로 붙어 온다** — `"… {slightly doubtful, slightly calm, very slightly interested}"`. 프로소디를 못 받는 LLM을 위한 Hume 기본 동작이고 **끄는 설정이 없다**(Hume Prompt Engineering 가이드). 우리는 `models.prosody.scores`를 쓰므로 **파싱 시점에 떼어낸다** (v1.13) |
...
| **세션 식별자 설정** | **앱이 `session_settings` 메시지로 보낸다.** 소켓 URL 쿼리로는 Hume까지 전달되지 않는다 (v1.10) |

> ⚠️ **`custom_session_id`를 소켓 URL 쿼리로 보내면 CLM까지 오지 않는다** (2026-09-07 실측). Hume이 인정하는 설정 경로는 **앱의 `session_settings` 메시지**와 **CLM 응답의 `system_fingerprint`** 둘뿐이고, 모르는 쿼리 파라미터는 조용히 무시한다. 위 표의 `?custom_session_id=`는 **값이 설정된 뒤에 Hume이 붙여 주는 것**이지 우리가 붙이는 것이 아니다.
>
> **이 실패는 연결 실패처럼 보이지 않는다.** 소켓은 정상으로 붙고 `chat_metadata`도 오고 `voice_session` 행도 생긴다 — CLM 호출에서만 값이 비어 401이 되고, EVI가 그 자리에서 대화를 끊는다(요청 2ms · 대화 길이 1.5초). 겉으로는 "붙었다가 끊긴다"로 보인다.
>
> ```json
> { "type": "session_settings", "custom_session_id": "{sessionId}", "audio": { … } }
> ```
>
> **소켓이 열린 직후, 오디오를 보내기 전에** 보낸다. 첫 발화의 CLM 호출부터 값이 실려야 한다.
...
| 버전 | 일자 | 내용 |
| --- | --- | --- |
| **v1.13** | **2026-09-17** | **`transcript`(§3-2)와 CLM `content`(§4)의 정의에 Hume 표정 꼬리를 명시** — `prosody-suffix-in-transcript.md`(백엔드) 회신. Hume은 user 발화 끝에 상위 3개 표정을 영문으로 덧붙이는데(`{slightly doubtful, …}`, 끄는 설정 없음), 우리 파서가 이를 그대로 들고 있어 **분석 입력·응답 이력·태그 대조·위기 규칙·백엔드 적재가 전부 오염**됐다. ① §4 표에 「`content`의 꼬리」 행 신설 ② §3-2에 `transcript` 행 신설 — **AI서버가 파싱 시점에 떼어낸 값**이 원문이고 백엔드는 고치지 않는다. **필드 추가·삭제·개명 없음** |
| **v1.12** | **2026-09-14** | **§2-2 `openSession`이 턴 0건 세션을 내주지 않는다** — `open-session-used-time.md`(앱) 회신. 앱은 「46초 대화 뒤 이어하기가 7분(`usedSec 0`)을 새로 준다」고 봤지만, **운영 DB와 Hume 채팅 기록을 맞춰 보니 `usedSec` 계산은 맞았고 카드가 가리킨 것은 종료 6~7초 뒤 생긴 턴 0건 세션**이었다(원인은 요약 라우트 중첩 — `request/app/summary-route-restarts-session.md`). 이어갈 대화가 없는 세션을 「중단된 대화」로 권하지 않도록 거른다. 기록 화면의 턴 0건 감춤 기준과 같다. **§2-2 표에 `usedSec` 정의(시작부터 마지막 턴까지)를 추가**했다. 「시작부터」만 있어 벽시계로 읽혔다. ⚠️ **벽시계 안은 택하지 않았다** — 앱이 죽은 뒤 흐른 시간은 Hume도 과금하지 않는데 빼면 TC-22가 깨진다. **필드 추가·삭제·개명 없음** |
| **v1.11** | **2026-09-08** | **`durationSec`의 정의를 §2-5에 명시하고 계산을 바로잡았다** — `duration-definition.md`(앱) 회신. 계약에는 예시(`214`)만 있고 무엇을 재는지 없었는데, **구현은 종료 경로마다 달랐다**: 사용자 종료는 벽시계, 스케줄러 정리는 `usedSec`. 벽시계 쪽은 **`hardCutSec` 420을 넘는 값까지 기록**했다(실측 951·630초 — 화면을 열어 두고 한참 뒤 말을 시작하면 그 공백이 전부 들어간다). ① 정의 = **첫 턴 ~ 마지막 턴**, `hardCutSec` 상한, 턴 0·1건이면 0 ② **`usedSec`(§2-2)과의 차이를 명시** — 시작부터 재는 값(잔여 시간)과 첫 턴부터 재는 값(화면 표시)은 다르다 ③ 두 종료 경로를 같은 정의로 통일. **하드컷 클램프만 쓰는 안은 기각** — 서로 다른 대화가 전부 정확히 420초로 찍혀 오류가 은폐된다. 실측 951초 → **31초**. **필드 변경 없음** |
| **v1.10** | **2026-09-07** | **§1-1의 오기 정정.** `custom_session_id`를 **EVI 소켓 URL**로 보내는 것처럼 적혀 있었고 앱이 그대로 구현했는데, **Hume은 그 경로로 받지 않는다.** 값이 CLM까지 오지 않아 §4의 401이 항상 걸리고 EVI가 대화를 끊었다(실측: 401 열 건 · 쿼리 없는 주소 · 2ms · 대화 길이 1.5초). ① §1-1 문장 정정 ② **§4에 「세션 식별자 설정」 행과 경고 신설** — 설정 경로는 `session_settings` 메시지와 `system_fingerprint` 둘뿐이고, 표의 `?custom_session_id=`는 **Hume이 붙여 주는 것**이다. **소켓은 붙고 `chat_metadata`도 오므로 연결 실패로 보이지 않는다**는 점을 함께 적었다. **필드 변경 없음** |
