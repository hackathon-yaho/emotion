...
| AI-06 | LLM 모델 (초기값, 전부 환경변수) | **2026-09-05 Google Gemini 무료 티어** — 분석 `gemini-3.5-flash-lite` · 응답 `gemini-3.8-flash` · 관찰 `gemini-3.8-flash` · 요약 `gemini-3.5-flash-lite` | 분석은 지연 예산 400ms, 응답은 한국어 품질과 TTFT의 균형, 배치는 지연이 없으니 최상위. §2.6 |
...
| 항목 | 내용 |
| --- | --- |
| 모델 | `AI_MODEL_RESPOND` (초기 `gemini-3.8-flash`), **`reasoning_effort: none`**, 스트리밍 |
...
### 실측 (2026-09-05, Gemini 무료 티어, 워밍업 후 · **턴 간격 15초** · SDK 재시도 0회)

| 구간 | p50 | p95 |
| --- | ---: | ---: |
| 분석 호출 | 935ms | 1,209ms |
| 응답 TTFT | 1,525ms | 1,996ms |
| **순차 합계** | **2,406ms** | **3,190ms** |

**목표(1.0초)의 2~3배다. Hume 양단은 여기 포함돼 있지도 않다.**

> **첫 측정은 틀렸다.** 같은 코드로 p95 12,500ms를 보고했었는데, 원인은 서버가 아니라
> **측정 방법**이었다. 8회를 **간격 없이 연속으로** 쏘는 바람에 429가 났고, SDK가
> 조용히 재시도하며 잠든 시간이 통째로 "지연"으로 기록됐다. 백엔드가 잡아 줬다
> (`request/ai/latency-diagnosis.md`).
>
> **그래서 지연을 잴 때는 두 가지를 지킨다** — ① 실제 대화 간격(15초 이상)을 둘 것
> ② `max_retries=0`으로 429를 지연에서 분리할 것. **연속 호출 벤치마크는 무료 티어에서
> 자기 자신을 측정한다.**

**`reasoning_effort`는 분석에 붙이지 않는다.** 붙이기 전에 이미 1초이고, `low`를 붙여도
개선이 없으며(1,017 → 983ms), `none`은 `gemini-3.5-flash-lite`가 400으로 거부한다.
응답 모델은 `none`을 받는데 flash-lite는 안 받는다 — **같은 벤더 안에서도 갈린다.**

**콜드 스타트는 별개이고 해결했다.** 첫 호출이 20초를 넘어서, 기동 시 네 모델을 한 번씩
깨운다(`AI_WARMUP_ON_START`). 사용자가 처음 말을 거는 순간이 가장 느리면 안 된다.

**AI 쪽에서 더 줄일 방법을 검토했고 전부 채택하지 않았다.**

| 방법 | 왜 안 하나 |
| --- | --- |
| 추측 실행(§2.7) | 재측정 후 조건이 더 이상 급하지 않다. 또한 그러나 **버려지는 호출도 분당 한도를 먹어** 무료 티어에서는 429를 더 자주 맞는다 |
| 응답 모델을 더 빠른 것으로 | 사용자가 실제로 듣는 문장이다. 되묻기의 자연스러움이 이 제품의 전부다 |
| 분석 타임아웃 축소 | 분석이 죽으면 갭이 없다. **시간을 줄이는 게 아니라 기능을 끄는 것**이다 |
| 분석을 한 턴 늦추기 | 되묻기가 다음 발화에 붙는다. "방금 그 말"이 아니면 되묻기가 아니다 |

**그래서 화면과 팀 결정에 올렸다** — `request/app/conversation-latency.md`. 유료 티어
전환 여부가 가장 큰 변수이고, 그건 여기서 정하지 않는다.
...
| 모델 | `AI_MODEL_OBSERVE` (초기 `gemini-3.8-flash`), `reasoning_effort: none` |
...
| AI-25 | **`reasoning_effort: none`** — 응답·관찰에서 사고를 끈다 | 켜 두면 사고 토큰이 출력 예산을 먹어 **문장이 중간에서 잘린다**(실측: `finish_reason=length`, 보이는 토큰 5개). 위기 응답에서 109가 잘려 나갔다 — 안전 경로가 조용히 깨지는 종류의 버그다. 출력 상한도 300 → 1000으로 올렸다 | 2026-09-05 |
| AI-26 | **기동 시 모델 워밍업** | 콜드 스타트가 20초를 넘는다(실측). 사용자가 처음 말을 거는 순간이 가장 느리면 안 된다. 실패해도 무시한다 — 기동 조건이 아니다 | 2026-09-05 |
| AI-27 | 잘못된 요청 본문은 **400**으로 끝낸다 | 500으로 두면 우리 서버 장애처럼 보이고 실제 장애와 로그에서 섞인다 | 2026-09-05 |
