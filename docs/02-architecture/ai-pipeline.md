...
### 실측 (2026-09-05, Gemini 무료 티어, 워밍업 후 8회)

| 구간 | p50 | p95 |
| --- | ---: | ---: |
| 분석 호출 | 4,106ms | 5,473ms |
| 응답 TTFT | 4,038ms | 7,027ms |
| **순차 합계** | **7,132ms** | **12,500ms** |

**목표(1.0초)의 7~12배다. Hume 양단은 여기 포함돼 있지도 않다.** 한가할 때 한 번만
부르면 1.9초에 끝나므로, 원인은 모델 자체가 아니라 **무료 티어의 분당 요청 제한**으로
보인다(워밍업 중 실제로 429를 받았다).
...
| 방법 | 왜 안 하나 |
| --- | --- |
| 추측 실행(§2.7) | 조건은 충족한다. 그러나 **버려지는 호출도 분당 한도를 먹어** 무료 티어에서는 429를 더 자주 맞는다 |
